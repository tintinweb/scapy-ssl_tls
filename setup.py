...
def get_site_packages():
  """
  This is a hack to work around site.getsitepackages() not working in virtualenv
...
def get_scapy_locations(sites):
  ...
  for site in sites:
    try:
      ...
      print("Non exisiting site-package reported: %s. Skipping" % site, file=sys.stderr)
    else:
      ...
      for dir_ in dirs:
        if dir_.startswith("scapy") and not dir_.endswith("egg-info"):
          ...
          if os.path.isdir(scapy_path):
            # Look for layers folder under the scapy install folder (can be nested)
    ...
    for line in fileinput.input(scapy_config, inplace=1, backup=".bak"):
      if line.strip().startswith("load_layers"):
        ...
        if processing_layer_list and line.strip().endswith("]"):
          # TODO, consider single quote strings, and consider lonely ] characters
          last_quote = line.rfind("\"")
          if last_quote > 0 and "ssl_tls" not in line:
            print("%s, \"ssl_tls\" ]" % line[:last_quote + 1], end="")
            processing_layer_list = False
          else:
            print(line)
            processing_layer_list = False
